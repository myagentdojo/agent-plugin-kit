...
| Which Failure Class value may a record carry? | Failure Class | Root [`CONTEXT.md`](CONTEXT.md) Failure Class entry, carried in proposed `docs/adr/0004-public-serialized-validation-and-transient-reconstruction.md` | Six primary meanings: `src/modules/maintenance-command-contract/interface.ts`; the observation-only `event_delivery`: `src/adapters/maintenance-command-facade/interface.ts` | `src/adapters/maintenance-command-facade/contract-tests/observability.test.ts` | Maintenance Command Contract for Maintenance Error; Maintenance Command Facade Adapter for record projection |
| Who validates a Public Serialized Value? | Public Serialized Value | Proposed `docs/adr/0004-public-serialized-validation-and-transient-reconstruction.md` | The existing Maintenance Command Contract, Maintenance Command Facade Adapter, or Qualification Evidence Interface that owns the value | The same owner's Contract Tests and Clean Fixture production-only resolution proof | The same existing owner; no shared schema owner |
| Who owns Transient Reconstruction semantics? | Transient Reconstruction and Logical Record | Proposed `docs/adr/0004-public-serialized-validation-and-transient-reconstruction.md` | `src/adapters/maintenance-command-facade/interface.ts` | `src/adapters/maintenance-command-facade/contract-tests/` and public-process evidence | Maintenance Command Facade Adapter; no retained reconstruction service |
