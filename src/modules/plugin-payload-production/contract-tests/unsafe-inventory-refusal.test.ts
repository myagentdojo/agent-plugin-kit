...
test("U10 bytes or modes changed after preparation are refused", async () => {
...
test("U13 a failing compressor reports no publication after being reached once", async () => {
  const subject = fixture()
...
test("U17 concurrent identical publications converge on one artifact set", async () => {
  const subject = fixture()
  const [first, second] = await Promise.all([produce(subject), produce(subject)])
...
test("U18 a conflicting candidate preserves the winner and refuses the loser", async () => {
  const subject = fixture()
  const other = { ...subject.request, sourceIdentity: { ...subject.request.sourceIdentity, commit: "2".repeat(40) }, prepared: { ...subject.request.prepared, sourceIdentity: { ...subject.request.sourceIdentity, commit: "2".repeat(40) } } }
  const outcomes = await Promise.all([produce(subject), produce(subject, {}, rebind(other))])
  ...
  if (winner?.kind === "packaged") expect(document.sourceCommit).toBe(winner.sourceIdentity.commit)
  expect(readdirSync(subject.distRoot).sort()).toEqual(["plugin-0.1.0.checksums.json", "plugin-0.1.0.tar.gz"])
