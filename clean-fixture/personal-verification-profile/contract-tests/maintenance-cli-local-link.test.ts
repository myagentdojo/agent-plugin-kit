...
test("foreign cwd fixed help uses an explicit fixed run ID", () => absent(localLinkContractSubject?.fixedHelpArgv, ["maintenance", "--json", "--run-id", "p3-local-link-help", "help"], "the linked public binary must emit fixed-run help outside both repositories"))
test("cleanup ledger pins exactly four public CLI executions", async () => {
  ...
  expect(observations?.[0]?.stdout).toContain('"run_id":"p3-local-link-help"')
  ...
  expect(observations?.[1]?.stderr).toContain('"run_id":"p3-local-link-usage"')
  expect(observations?.[2]?.exitCode).toBe(literalHelpProcess.exitCode)
  expect(observations?.[2]?.stdout).toContain('"run_id":"p3-local-link-event"')
