...
test("CF02 host-temporary fixture materializes through the real process and converges", async () => {
  ...
  for (const path of [".claude-plugin/marketplace.json", ".agents/plugins/marketplace.json", "plugin/.claude-plugin/plugin.json", "plugin/.agents/plugin.json", "plugin/skill-inventory.json", "plugin/runtime/bundle-inventory.json", "plugin/runtime/bundle-inventory.sh", "plugin/THIRD-PARTY-NOTICES.md"]) expect(statSync(join(subject.root, path)).isFile()).toBe(true)
  expect(statSync(join(subject.pluginRoot, "runtime/bundle-inventory.sh")).mode & 0o111).not.toBe(0)
