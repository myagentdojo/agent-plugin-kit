...
test("CF04 public process bytes, streams, exits, refusal, ingress, and repair are stable", async () => {
  ...
  const manifestPath = join(consumer.consumerRoot, "package.json")
  const originalManifest = readFileSync(manifestPath, "utf8")
  try {
    const manifest = JSON.parse(originalManifest)
    writeFileSync(manifestPath, JSON.stringify({ ...manifest, version: "0.1.0" }))
    successEnvelope(await runPayload(subject, "materialize", "cf04-metadata-materialize"))
    successEnvelope(await runPayload(subject, "check", "cf04-metadata-check"))
    const packageRequest = join(subject.root, "package.json")
    writeFileSync(packageRequest, JSON.stringify(subject.request))
    const packaged = await consumer.run(["--run-id", "cf04-metadata-package", "maintenance", "payload", "package", "--request", packageRequest])
    expect(packaged.exitCode).toBe(2)
    expect(packaged.stdout).toBe("")
    expect(packaged.stderr).toContain("Maintenance source checkout is not admitted.")
    writeFileSync(manifestPath, JSON.stringify({ ...manifest, dependencies: { "agent-plugin-kit": `git+https://github.com/myagentdojo/agent-plugin-kit.git#${"e".repeat(40)}` } }))
    const changedPin = await runPayload(subject, "check", "cf04-changed-pin")
    expect(changedPin.exitCode).toBe(2)
    expect(changedPin.stdout).toBe("")
    expect(changedPin.stderr).toContain("Maintenance source checkout is not admitted.")
  } finally {
